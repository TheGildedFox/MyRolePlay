...
local function clean( x )
	return x:gsub("||", "|"):gsub("|n", ""):gsub("|c%x%x%x%x%x%x%x%x", ""):gsub("|r", ""):gsub("|H.-|h(.-)|h", "%1"):gsub("|T.-|t", ""):gsub("|K.-|k.-|k", ""):gsub("|", "||"):trim()
end
...
mrp.DisplayTooltip = setmetatable( {
	["VA"] = function( contents )
		if emptynil( contents ) then
			...
			if strfind( contents, "XRP/", 1, true ) then
				tinsert(s, "XRP")
			end
			if strfind( contents, "TotalRP3/", 1, true ) then
				tinsert(s, "TRP3")
			end
